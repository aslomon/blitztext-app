# Blitztext — Autonomous Development Backlog
...
- [x] **R3-UX-micro** (Iter 19) Verbesserungen-Microcopy — "Vorher/Nachher" is ambiguous (Vorher=inserted Blitztext, Nachher=user edit); relabel "Eingefügt → Deine Korrektur" + optionally hide unchanged rows (ImprovementSection.swift) — `refl-ux`
...
- [ ] **R4-FT-automation** [P1 M] Local automation surface — AppIntents (Shortcuts/Spotlight/Raycast) + `blitztext://` URL scheme for "dictate in mode X" / "rewrite selection in mode X" / "open archive". The agent-native angle the brand promises but the code has zero of; no new always-on server — `refl-features`
...
- [x] **FT-8 In-App-Updates (Sparkle)** Daily + manual update checks with full auto-install: `UpdateService` (gentle reminders for the LSUIElement app), Updates section in System settings, popover footer hint, nested Sparkle signing in build.sh, EdDSA keys, release pipeline (`release.yml`, `docs/release-process.md`), privacy docs. 327 tests green, universal build verified. Remaining for public distribution: Developer ID + notarization (`docs/app-store-runbook.md`). — `user`
- [x] **SPINOFF-rede** The public product now lives in the private repo `aslomon/rede-app` (lowercase wordmark "rede", bundle `app.rede.mac`, v1.0, own EdDSA key + appcast feed, speech-bubble icon with coral accent, MIT attribution surface). Features land in this fork first, then merge into rede (`blitztext` remote). — `user`
