...
## Status (2026-06-10, end of autonomous run)

- **Phase 1 (in-app updater): DONE & verified** — Sparkle 2.9.3, `UpdateService` with gentle
  reminders, Updates + Über & Lizenzen sections, popover hints, nested Sparkle signing,
  EdDSA public key pinned. 327 tests green; universal debug build verified. Open: one manual
  end-to-end update-cycle GUI test against a local appcast (release-process.md §Verifying).
- **Phase 2 (pipeline): DONE as scaffolding** — `release.yml` + runbook; fork keypair in the
  login Keychain (`generate_keys`), public key in Info.plist. Open: first real tagged release,
  gh-pages branch + Pages activation, `SPARKLE_PRIVATE_KEY` CI secret, key backup.
- **Phase 4 (was ist neu): partly shipped** — gentle-reminder badge + release-notes flow;
  post-update "Was ist neu" panel still open (4.2).
- **Phase 5 (`rede` spin-off): DONE** — private repo `aslomon/rede-app` (bundle `app.rede.mac`,
  product `rede` v1.0, module pinned to `Blitztext` for cheap merges, own EdDSA key
  `--account rede`, own feed URL, speech-bubble icon w/ coral accent `#FF5C4D` generated by
  `scripts/generate-icons.swift`, rede DESIGN/README, origin notes). 327 tests green,
  `rede.app` builds universal. Open: `scripts/create-dev-cert.sh` once for a stable
  "rede Local Dev" identity on the dev machine.
- **Phase 3 / 6: open, gated on the Apple Developer account** — every account-dependent step
  is itemized in `docs/app-store-runbook.md`.
