...
How a Blitztext release is built, signed, published, and picked up by the in-app updater.
The update feed lives on this fork (`aslomon/blitztext-app`): GitHub Releases hold the signed
...
- Both live in `BlitztextMac/project.yml`.
...
cert ("Blitztext Local Dev", installed via `scripts/create-dev-cert.sh`):

```bash
# 1. Bump MARKETING_VERSION + CURRENT_PROJECT_VERSION in BlitztextMac/project.yml; commit.
...
ditto -c -k --sequesterRsrc --keepParent Blitztext.app Blitztext-<version>.zip

# 4. EdDSA-sign the archive (key from Keychain):
./sparkle-tools/bin/sign_update Blitztext-<version>.zip
#    → sparkle:edSignature="…" length="…"

# 5. Tag + release on the fork:
git tag v<version> && git push fork v<version>
gh release create v<version> --repo aslomon/blitztext-app --title "Blitztext v<version>" \
  --notes "<German release notes from CHANGELOG>" Blitztext-<version>.zip
...
<item>
  ...
  <enclosure
    url="https://github.com/aslomon/blitztext-app/releases/download/v<version>/Blitztext-<version>.zip"
...
3. Serve a local appcast: put `appcast.xml` + zip into a folder,
   ...
   `defaults write app.blitztext.mac SUFeedURL http://localhost:8000/appcast.xml`.
...
5. Remove the `defaults` override afterwards: `defaults delete app.blitztext.mac SUFeedURL`.
